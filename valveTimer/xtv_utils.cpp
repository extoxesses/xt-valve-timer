...
static void XtvUtils::checkTimer(RtcDateTime& now, Valve* valves, short valvesSize) {
...
static char* XtvUtils::formatDate(const RtcDateTime* now) {
  short arrSize = 11;
...
static char* XtvUtils::formatTime(const RtcDateTime* now) {
  short arrSize = 6;
  char* dateString = new char[arrSize];
  short stringSize = arrSize * 8; // char sizeOf
...
static Valve* XtvUtils::getValve() {
  short idx = max(0, NAV_PTR[0] - 1);
  return &valves[idx];
}

static const char* XtvUtils::parseBoolean(bool value) {
  return value ? "On" : "Off";
}
